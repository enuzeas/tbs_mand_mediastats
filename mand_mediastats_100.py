...
def mediastats(pgmkey, index):
    ...
    yesterday_mk = datetime.date.strftime(datetime.date.today()- datetime.timedelta(days=index),'%Y-%m-%d 00:00:00')
...
for i in range(1,100):
	tabs = [mediastats(key,i) for key in pgm_key.keys()]
	yesterday = datetime.date.strftime(datetime.date.today()- datetime.timedelta(days=i),'%Y-%m-%d')
	result = pd.concat(tabs, ignore_index =True)
	result.to_csv('/home/enuzeas/tbs_mand_mediastats/csv_/mediastat_{}.csv'.format(yesterday),index =False)
	print("created mediastat_{}.csv".format(yesterday))
